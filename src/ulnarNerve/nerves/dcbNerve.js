...
import { selectDCB, removeDCB } from '../../actions/selectUlnarMuscle';

class DcbNerve extends Component {
    constructor() {
        super();
        this.state = {
            shadowOpacity: 0,
    ...
    handleNerveClick = () => {
        this.props.selectNerve('Dorsal Cutaneous Branch');
        this.props.setCursorOpacity();
        setTimeout(() => {
            this.props.removeCursorOpacity();            
    ...
    render() {
        ...
        return (
            <Group>
                <Line
                    points={[680, 260, 735, 290, 780, 400, 870, 450]}
                    stroke={'#fff'}
                    strokeWidth={15}
                    lineCap={'bevel'}
                    lineJoin={'bevel'}
...
const mapStateToProps = (state) => {
    return {
        ulnarMuscleColor: state.ulnarMuscleColor.DCB
    }
}

const mapDispatchToProps = (dispatch) => {
   return bindActionCreators({ selectNerve, openNerveModal, setCursorOpacity, removeCursorOpacity, selectDCB, removeDCB, setCursorTextOpacity, removeCursorTextOpacity }, dispatch); 
};

export default connect(mapStateToProps, mapDispatchToProps)(DcbNerve);