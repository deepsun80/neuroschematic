...
import { selectPQ, removePQ } from '../../actions/selectMedianMuscle';

class PqNerve extends Component {
    constructor() {
        super();
        this.state = {
            shadowOpacity: 0,
    ...
    render() {
        ...
        return (
            <Group offsetX={-110}>
                <Line
                    points={[985, 360, 1060, 370, 1100, 420, 1170, 430]}
                    stroke={'#fff'}
                    strokeWidth={12}
                    lineCap={'bevel'}
                    lineJoin={'bevel'}
...
const mapStateToProps = (state) => {
    return {
        medianMuscleColor: state.medianMuscleColor.PQ
    }
}

const mapDispatchToProps = (dispatch) => {
   return bindActionCreators({ selectNerve, openNerveModal, setCursorOpacity, removeCursorOpacity, selectPQ, removePQ }, dispatch); 
};

export default connect(mapStateToProps, mapDispatchToProps)(PqNerve);