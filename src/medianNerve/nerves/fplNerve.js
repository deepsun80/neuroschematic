...
import { selectFPL, removeFPL } from '../../actions/selectMedianMuscle';

class FplNerve extends Component {
    constructor() {
        super();
        this.state = {
            shadowOpacity: 0,
    ...
    render() {
        ...
        return (
            <Group offsetX={-60}>
                <Line
                    points={[985, 360, 1070, 375, 1090, 430]}
                    stroke={'#fff'}
                    strokeWidth={12}
                    lineCap={'bevel'}
                    lineJoin={'bevel'}
...
const mapStateToProps = (state) => {
    return {
        medianMuscleColor: state.medianMuscleColor.FPL
    }
}

const mapDispatchToProps = (dispatch) => {
   return bindActionCreators({ selectNerve, openNerveModal, setCursorOpacity, removeCursorOpacity, selectFPL, removeFPL }, dispatch); 
};

export default connect(mapStateToProps, mapDispatchToProps)(FplNerve);